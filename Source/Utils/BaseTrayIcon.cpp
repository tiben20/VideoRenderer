...
BOOL CBaseTrayIcon::ProcessBlackList()
{
    ...
    for (int i = 0; i < (sizeof(noTrayProcesses) / sizeof(noTrayProcesses[0])) ; i++)
    {
...
HRESULT CBaseTrayIcon::OpenPropPage()
{
    ...
    CheckPointer(m_pFilter, E_INVALIDARG);
    CoInitialize(nullptr);

    // Get PropertyPages interface
    ISpecifyPropertyPages* pProp = nullptr;
    HRESULT hr = m_pFilter->QueryInterface<ISpecifyPropertyPages>(&pProp);
    if (SUCCEEDED(hr) && pProp)
    {
      ...
      m_pFilter->QueryInterface<IUnknown>(&pFilterUnk);
