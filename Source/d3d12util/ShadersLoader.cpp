...
UINT ResolvePasses(const std::vector<std::string_view>& blocks, const std::vector<std::string_view>& commons, ShaderDesc& desc) {
	// optional: BIND, SAVE
	...
	{
		if (FAILED(CompileShader(passSources[0], "__M", "ps_5_0",&desc.passes[0].cso, "Pass1")))
		{
	...
	else 
	{
		...
		for (size_t i = 0; i < passSources.size(); ++i) {
			if (FAILED(CompileShader(passSources[i], "__M", "ps_5_0", &desc.passes[i].cso, "Pass1")))
			{
				DLog(L"Failed compiling passes for current scaler");
			}
		}
...
bool CShaderFileLoader::Compile(ShaderDesc& desc)
{
  std::wstring currentpath;
  currentpath = _wgetenv(L"APPDATA");

  currentpath.append(L"\\MPCVideoRenderer\\");
  currentpath.append(m_pFile);
  std::wstring currentsource;
  std::wifstream infile(currentpath.c_str());
  std::string source;
  
  if (!Utility::ReadTextFile(currentpath.c_str(), source))
  {
    DLog(L"couldn't read for compile {}", currentpath.c_str());
    return false;
  }
