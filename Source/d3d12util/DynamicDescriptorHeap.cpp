...
std::vector<CComPtr<ID3D12DescriptorHeap>> DynamicDescriptorHeap::sm_DescriptorHeapPool[2];
...
ID3D12DescriptorHeap* DynamicDescriptorHeap::RequestDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE HeapType)
{
    ...
    else
    {
        ...
        HeapDesc.NodeMask = 1;
        CComPtr<ID3D12DescriptorHeap> HeapPtr;
        EXECUTE_ASSERT(S_OK == g_Device->CreateDescriptorHeap(&HeapDesc, MY_IID_PPV_ARGS(&HeapPtr)));
